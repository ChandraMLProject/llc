...
MODEL_FILE = "model.h5"
MODEL_URL = "https://www.dropbox.com/scl/fi/j4j9v19qqyb5ih08n93wc/model.h5?rlkey=ybbzgbiybo1c9cr8d9a42efy6&dl=1"
...
def download_model():
    if not os.path.exists(MODEL_FILE) or not is_valid_h5(MODEL_FILE):
        ...
        if "html" in response.headers.get("Content-Type", ""):
            st.error("Model download failed: received HTML instead of a binary file.")
            return
...
def initialize_model():
    ...
    if not is_valid_h5(MODEL_FILE):
        st.error("Model file is not valid. Please check the model source.")
        st.stop()
